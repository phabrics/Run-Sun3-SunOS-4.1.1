...
tme_m68k_write_memx16(struct tme_m68k *ic) 
{
  ...
  if (__tme_predict_true(!TME_M68K_SEQUENCE_RESTARTING
...
#if 1
    {
	    ...
	    if ((int)ic->tme_m68k_ireg_pc == 0x3c3bc) {
	//	    printf("tme_m68k_write_memx16 #1; mem %x, val %x\n", (int)mem, (int)mem_value);
	    }
    }
#endif
  ...
  /* otherwise, do the bus cycles the slow way: */
...
#if 1
    {
	    ...
	    if ((int)ic->tme_m68k_ireg_pc == 0x3c3bc) {
		    mem = (tme_shared tme_uint16_t *) (tlb->tme_m68k_tlb_emulator_off_write + linear_address_first);
		    mem_value = tme_htobe_u16(ic->tme_m68k_ireg_memx16);
//		    printf("tme_m68k_write_memx16 #2; mem %x, val %x\n", (int)mem, (int)mem_value);
	    }
    }
#endif
  ...
  int should_fault = 0;
    {
	    extern int printf(const char *format, ...);
	    if ((int)ic->tme_m68k_ireg_pc == 0x3c3bc) {
//		    printf("tme_m68k_write; *_linear_address %x\n", (int)*_linear_address);
		    if ((int)*_linear_address == 0x509000) {
			    should_fault = 1;
		    }
//		    printf("tme_m68k_write; should_fault %d\n", should_fault);
	    }
    }
#endif
  ...
  /* otherwise, a later transfer has faulted.  just step the
...
#if 0
    {
	    extern int printf(const char *format, ...);
	    if ((int)ic->tme_m68k_ireg_pc == 0x3c3bc) {
		    printf("tme_m68k_write; returning\n");
	    }
    }
#endif
    return;
  ...
  for(; transferred < reg_size; ) {
...
#if 1
    {
	    extern int printf(const char *format, ...);
	    if ((int)ic->tme_m68k_ireg_pc == 0x3c3bc) {
//		    if (0) printf("tme_m68k_write; not returning\n");
		    if (should_fault)  {
//			    printf("tme_m68k_write; faulting\n");
			    exception = TME_M68K_EXCEPTION_BERR;
		    }
	    }
    }
